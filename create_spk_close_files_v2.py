...
2. Get the same speakers from test_erj_spk_open_test.json from unlabeled data, but remove duplicated ones
"""
...
def main():
    ...
    if not test_data or not unlabeled_close_data:
    ...
    # Get files from unlabeled data that have the same speakers as test
    for file_id, item in unlabeled_close_data.items():
    ...
    # Print breakdown by speaker for labeled data
    print("\nBreakdown by speaker for labeled close training:")
    speaker_counts = {}
    for item in labeled_close_train.values():
