...
class ASR(sb.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            # remove sil for valid compare
            silence_id = self.label_encoder.lab2ind["sil"]
            
            # pop silences
            sequence = [
                [phn for phn in seq if phn != silence_id] for seq in sequence
            ]
               
            # pop silences
            canonicals = [
                [phn for phn in seq if phn != silence_id] for seq in canonicals
            ]
